...
const getStatusLabel = (record: PTPackageRecord): string => {
...
const getFilterSuffix = (filters?: PTPackageRecordFilters): string => {
...
export const exportToExcel = (records: PTPackageRecord[]) => {
  // Create CSV content
  const headers = [
    "Invoice",
    "Member ID",
  ...
  // Create filename
  const filename = `pt-package-records-${format(new Date(), "yyyy-MM-dd")}.csv`;
...
export const exportToPDF = (records: PTPackageRecord[], filters?: PTPackageRecordFilters) => {
